...
namespace eBACSMobileV2
{
    ...
    public class ReadingActivity : Activity
    {
        ...
        TextView othercharge, meteringfee, totalamountdue, finishcount,seniorlabel,seniordisc,advancepayment, arrears, franchisetax, seweragefee, franchiselabel, seweragelabel, otherchargelabel, arrearslabel, advpaymentlabel;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            franchisetax = FindViewById<TextView>(Resource.Id.txtfranchisetax);
            seweragefee = FindViewById<TextView>(Resource.Id.txtseweragefee);
            franchiselabel = FindViewById<TextView>(Resource.Id.lblfranchisetax);
            seweragelabel = FindViewById<TextView>(Resource.Id.lblseweragefee);
            otherchargelabel = FindViewById<TextView>(Resource.Id.lblothercharges);
            arrearslabel = FindViewById<TextView>(Resource.Id.lblarrears);
            advpaymentlabel = FindViewById<TextView>(Resource.Id.lbladvpayment);
            ...
            else
            {
                ...
                using (var connection = new SQLiteConnection(System.IO.Path.Combine(folder, "eBacsMobile.db")))
                {
                    try
                    {
                        ...
                        DevDue = devCdate.AddDays(14); //due date
                        string stringddue;
                        ...
                        connection.Query<tblbillchargesSQLite>("Update tblbillchargesSQLite set Amount=? WHERE BillNumber =? AND Particulars =?", seweragefee.Text, billno.Text, "Sewerage Fee");

                        connection.Query<tblbillchargesSQLite>("Update tblbillchargesSQLite set Amount=? WHERE BillNumber =? AND Particulars =?", franchisetax.Text, billno.Text, "Franchise Tax");
                ...
                try
                {
                    ...
                    using (var connection = new SQLiteConnection(System.IO.Path.Combine(folder, "eBacsMobile.db")))
                    {
                        ...
                        if (consumption <= 10)
                        {
                            ...
                            penal.Text = ((double.Parse(amountdue.Text) + double.Parse(arr)) * 0.05).ToString();
                        }
                        else
                        {
                            ...
                            penaltytwodecimal =((double.Parse(amountdue.Text) + double.Parse(arr)) * 0.05);
        ...
        private void LoadData()
        {
            ...
            try
            {
                ...
                Console.WriteLine("THIS: " + advancepayment.Text);
                Console.WriteLine("THIS: " + arrears.Text);
        ...
        private void computee()
        {
            using (var connection = new SQLiteConnection(System.IO.Path.Combine(folder, "eBacsMobile.db")))
            {
                ...
                //get franchise tax
                var qryFranchiseTax = connection.Query<tblbillchargesSQLite>("select * from tblbillchargesSQLite where BillNumber=? AND Particulars = 'Franchise Tax'", billno.Text);
                //Android.Widget.Toast.MakeText(Android.App.Application.Context, chargeslist.Count(), ToastLength.Long).Show();
                if (qryFranchiseTax.Count == 0)
                {

                    franchisetax.Text = "0";
                    //franchisetax.Visibility = Android.Views.ViewStates.Invisible;
                    franchisetax.Visibility = ViewStates.Gone;
                    franchiselabel.Visibility = ViewStates.Gone;
                ...
                else
                {
                    decimal chargelisttt;
                    chargelisttt = 0;
                    for (int iii = 0; iii < qryFranchiseTax.Count; iii++)
                    {
                        chargelisttt = chargelisttt + qryFranchiseTax[iii].ChargeRate;

                    }
                    chargelisttt = (chargelisttt/ 100) * decimal.Parse(amountdue.Text);

                    franchisetax.Text = chargelisttt.ToString("N2", CultureInfo.InvariantCulture);
                    franchisetax.Visibility = ViewStates.Visible;
                    franchiselabel.Visibility = ViewStates.Visible;


                }

                //get sewerage fee
                var qrySewerageFee = connection.Query<tblbillchargesSQLite>("select * from tblbillchargesSQLite where BillNumber=? AND Particulars = 'Sewerage Fee'", billno.Text);
                //Android.Widget.Toast.MakeText(Android.App.Application.Context, chargeslist.Count(), ToastLength.Long).Show();
                if (qrySewerageFee.Count == 0)
                {

                    seweragefee.Text = "0";
                    seweragefee.Visibility = ViewStates.Gone;
                    seweragelabel.Visibility = ViewStates.Gone;
                }
                else
                {
                    decimal chargelisttt;
                    chargelisttt = 0;
                    for (int iii = 0; iii < qrySewerageFee.Count; iii++)
                    {
                        chargelisttt = chargelisttt + qrySewerageFee[iii].ChargeRate;

                    }

                    chargelisttt = chargelisttt * decimal.Parse(currentconsumption.Text);

                    seweragefee.Text = chargelisttt.ToString("N2", CultureInfo.InvariantCulture);
                    seweragefee.Visibility = ViewStates.Visible;
                    seweragelabel.Visibility = ViewStates.Visible;
                }

                var otherchargeslist = connection.Query<tblbillchargesSQLite>("select * from tblbillchargesSQLite where BillNumber=? AND NOT Particulars = 'Metering Fee' AND NOT Particulars = 'Sewerage Fee' AND NOT Particulars = 'Franchise Tax'", billno.Text);
                //Android.Widget.Toast.MakeText(Android.App.Application.Context, chargeslist.Count(), ToastLength.Long).Show();
                if (otherchargeslist.Count == 0)
                {
                    othercharge.Text = "0";
                    othercharge.Visibility = ViewStates.Gone;
                    otherchargelabel.Visibility = ViewStates.Gone;
                }
                else
                {
                    ...
                    othercharge.Visibility = ViewStates.Visible;
                    otherchargelabel.Visibility = ViewStates.Visible;
                }

                if (arrears.Text == "0")
                {
                    arrearslabel.Visibility = ViewStates.Gone;
                    arrears.Visibility = ViewStates.Gone;

                }
                else
                {
                    arrearslabel.Visibility = ViewStates.Visible;
                    arrears.Visibility = ViewStates.Visible;
                }

                if (advancepayment.Text == "0")
                {
                    advpaymentlabel.Visibility = ViewStates.Gone;
                    advancepayment.Visibility = ViewStates.Gone;
                }
                else
                {
                    advpaymentlabel.Visibility = ViewStates.Visible;
                    advancepayment.Visibility = ViewStates.Visible;
                }
            ...
            {
                totalamountdue.Text = (double.Parse(amountdue.Text) + double.Parse(arrears.Text) + double.Parse(othercharge.Text) + double.Parse(meteringfee.Text) + double.Parse(franchisetax.Text) + double.Parse(seweragefee.Text) - double.Parse(seniordisc.Text) - double.Parse(advancepayment.Text)).ToString("N2", CultureInfo.InvariantCulture);
            }
            catch
            {
        ...
        private void printbill()
        {
            ...
            try
            {
                ...
                decimal currentbillingdecimal, meteringfeedecimal, seniordecimal, amountduedecimal, penaltydecimal, advancedecimal, arrearsdecimal, franchisedecimal, seweragedecimal;
                ...
                franchisedecimal = decimal.Parse(franchisetax.Text);
                seweragedecimal = decimal.Parse(seweragefee.Text);
                ...
                stringBuilder.Append("         SANTA ROSA (NE) WATER DISTRICT\n");
                stringBuilder.Append("        Santa Rosa - Fort Magsaysay Road\n");
                stringBuilder.Append("            Santa Rosa, Nueva Ecija\n");
                ...
                stringBuilder.Append("     Due Date          |" + "      Discon Date\n");
                ...
                if (franchisedecimal > 0)
                {
                    stringBuilder.Append("Franchise Tax            : " + String.Format("{0, 20}", "" + franchisedecimal.ToString("N2", CultureInfo.InvariantCulture) + "\n"));
                }

                if (seweragedecimal > 0)
                {
                    stringBuilder.Append("Sewerage Fee             : " + String.Format("{0, 20}", "" + seweragedecimal.ToString("N2", CultureInfo.InvariantCulture) + "\n"));
                }
                ...
                if (advancedecimal > 0)
                {
                    ...
                    //stringBuilder.Append("Advance Payment               : ");
                    //stringBuilder.Append("" + advancedecimal.ToString("N2", CultureInfo.InvariantCulture) + "\n");
                ...
                stringBuilder.Append("Penalty After Due        : " + String.Format("{0, 20}", "" + penaltydecimal.ToString("N2", CultureInfo.InvariantCulture) + "\n"));
